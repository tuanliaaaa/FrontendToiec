...
async function renResultPage(){
    ...
    let response = await getAjax('http://127.0.0.1:8080/api/v1/histories/lessonbypart/85',localStorage.getItem("access_token"));
    ...
    if (response.status >= 200 && response.status < 300) {
        ...
        response.data.data.questionGroups.map((item,index)=>{
            ...
            resultHtml+= `
                <div class="mid__item ${(index === 0) ? 'active' : ''}" data-index="${index}" >
